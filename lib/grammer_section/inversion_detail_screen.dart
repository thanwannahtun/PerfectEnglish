...
class InversionDetailScreen extends StatelessWidget {
  ...
  final List<Map<String, dynamic>> inversionData = const [
    {
      ...
      'desc': 'ဝါကျကို အလေးအနက်ပြုလိုသောအခါ "ဘယ်သောအခါမှ၊ ခဲခဲယဉ်ယဉ်" စသည့် စကားလုံးများကို ဝါကျအစတွင်ထားပါက Inversion လုပ်ရမည်။',
      ...
      'examples': [
        {
          ...
          'mm': 'ဤမျှလှပသော မြင်ကွင်းမျိုးကို တစ်ခါဖူးမျှ ကျွန်ုပ်မမြင်ဖူးခဲ့ပါ။',
    ...
    },
    {
      ...
      'desc': '"Only" ဖြင့်စသော အသုံးအနှုန်းများသည် ဝါကျ၏ အစတွင်ရှိနေပါက ဒုတိယဝါကျပိုင်း (Main Clause) တွင် Inversion ဖြစ်ပေါ်သည်။',
      ...
      'examples': [
        {
          ...
          'mm': 'ထိုအချိန်ရောက်မှသာလျှင် သူသည် ပြဿနာကို နားလည်သဘောပေါက်သွားတော့သည်။',
    ...
    },
    {
      ...
      'desc': 'အဆင့်မြင့်အရေးအသားတွင် "If" ကို ဖြုတ်ပြီး Inversion ပုံစံဖြင့် ပိုမိုခန့်ညားအောင် ရေးသားနိုင်သည်။',
      'examples': [
        {
        ...
        },
        {
          ...
          'mm': 'ကျွန်ုပ်သာ သင်၏နေရာတွင်ဆိုပါက ထိုကမ်းလှမ်းချက်ကို လက်ခံလိုက်မည်။',
          'logic': 'Were + I + you'
        },
        {
    ...
    },
    {
      ...
      'desc': 'တစ်ခုခုကို လုံးဝမသိလိုက်ခြင်း သို့မဟုတ် မမျှော်လင့်ထားခြင်းကို ဖော်ပြရာတွင် သုံးသည်။',
      'examples': [
        {
          'standard': 'He knew little about the surprise party.',
          'inversion': 'Little did he know about the surprise party.',
          'mm': 'ထိုအံ့ဩဖွယ်ရာ ပါတီအကြောင်းကို သူ လုံးဝ (စိုးစဉ်းမျှ) မသိခဲ့ချေ။',
  ...
  Widget _buildInversionFormulaHeader() {
    return Container(
      ...
      child: Column(
        children: [
          const Text("The Inversion Equation", style: TextStyle(color: Colors.amber, fontWeight: FontWeight.bold, fontSize: 18)),
          ...
          const Text("ပုံမှန်ဝါကျကို အမေးဝါကျပုံစံ ပြောင်းလဲရေးသားခြင်း ဖြစ်သည်။",
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 10),
            Text(data['desc'], style: const TextStyle(fontSize: 14, color: Colors.black54)),
            const SizedBox(height: 15),
            if (data.containsKey('words'))
              Wrap(
                spacing: 8,
                children: (data['words'] as List<String>).map((w) => Chip(
                  label: Text(w, style: const TextStyle(color: Colors.white, fontSize: 12)),
                  backgroundColor: Colors.indigoAccent,
                )).toList(),
              ),
            const Divider(height: 30),
            ... (data['examples'] as List).map((ex) => Container(
              margin: const EdgeInsets.only(bottom: 15),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text("Standard: ${ex['standard']}", style: const TextStyle(color: Colors.grey, fontSize: 13)),
                  const SizedBox(height: 4),
                  Text("Inverted: ${ex['inversion']}", style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.redAccent)),
                  const SizedBox(height: 4),
                  Text(ex['mm'], style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500)),
                  const SizedBox(height: 6),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                    decoration: BoxDecoration(color: Colors.green.shade50, borderRadius: BorderRadius.circular(5)),
                    child: Text("Logic: ${ex['logic']}", style: const TextStyle(fontSize: 12, color: Colors.green, fontWeight: FontWeight.bold)),
                  ),
                ],
              ),
            )),
    ...
    return Container(
      ...
      child: const Row(
        children: [
          ...
          SizedBox(width: 10),
          Expanded(child: Text("သတိပြုရန် - Inversion ကို ပုံမှန်စကားပြောတွင် အသုံးနည်းသည်။ စာမေးပွဲအဖြေလွှာများ၊ အရေးအသားများနှင့် မိန့်ခွန်းများတွင်သာ အဓိကထား သုံးစွဲသင့်သည်။", style: TextStyle(fontSize: 12, color: Colors.red))),
        ],
      ),
    );
  }
